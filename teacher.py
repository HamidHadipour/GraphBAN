...
class Model(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.encoder2 = to_hetero(self.encoder1, train_data_u.metadata(), aggr='sum')
...
def run_model(epochs, output_file_path, data_file_path):
  ...
  cp_mapping = load_node_csv(
  cp_path, index_col='SMILES'
    )
  cp_mapping_p = load_node_csv(
  ...
  edge_index, edge_label = load_edge_csv(
    ...
    src_mapping=cp_mapping,
    dst_index_col='Protein',
    dst_mapping=cp_mapping_p
    )

  data = HeteroData()
  data['chem'].num_nodes =len(cp_mapping)  # Users do not have any features.
  data['protein'].num_nodes =len(cp_mapping_p)
  data['chem', 'CPI', 'protein'].edge_index = edge_index.type(torch.int64)
  ...
  data['chem', 'CPI', 'protein'].edge_label_index = edge_label
  ...
  for epoch in range(1, epochs + 1):
    ...
    train_rmse, emb, ed = test_auroc(model, train_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="output.parquet", help="Path to save the output file.")
    parser.add_argument("--data_file_path", type=str, required=True, help="Path to the data file.")
    args = parser.parse_args()

    run_model(args.epochs, args.output, args.data_file_path)
