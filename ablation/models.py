...
class GraphBAN(nn.Module):
    def __init__(self, **config):
        ...
        self.ConcatenateApplyWeightNorm = ConcatenateApplyWeightNorm(128,128,256)
    ...
    def forward(self, bg_d, bg_smiles, v_p,v_p_esm, device, mode="train"):
        ...
        v_fusion = v_fusion.mean(dim=1)
        v_p_fusion = v_p_fusion.mean(dim=1)
        #f = self.ConcatenateApplyWeightNorm(v_fusion, v_p_fusion)
        #v_d = v_d.mean(dim=1)
        #v_p = v_p.mean(dim=1)
        f = torch.cat((v_fusion, v_p_fusion), dim = 1)
...
class ConcatenateApplyWeightNorm(nn.Module):
    def __init__(self, input_dim1, input_dim2, output_dim, concat_dim=1):
        """
        ...
        """
        super(ConcatenateApplyWeightNorm, self).__init__()
        self.concat_dim = concat_dim
        # Define a linear layer to apply after concatenation
        self.linear = nn.Linear(input_dim1 + input_dim2, output_dim)
        # Apply weight normalization to the linear layer
        self.weight_norm_layer = weight_norm(self.linear)
    ...
    def forward(self, tensor1, tensor2):
        ...
        concatenated = torch.cat((tensor1, tensor2), dim=self.concat_dim)
        # Apply the weight normalized linear layer
        output = self.weight_norm_layer(concatenated)
        return output