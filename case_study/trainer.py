...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader,output_folder, opt_da=None, discriminator=None,
        ...
        self.test_metrics = {}
        self.config = config
        self.output_dir = output_folder#config["RESULT"]["OUTPUT_DIR"]
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            val_loss= self.test(dataloader="val")
            if self.experiment:
        ...
        test_loss   = self.test(dataloader="test")
    ...
    def train_da_epoch(self):
        ...
        for i, (batch_s, batch_t) in enumerate(tqdm(self.train_dataloader)):
            ...
            v_d, v_p, f, score = self.model(v_d,sm, v_p,esm, device)
              ...
              #v_d = v_d.detach().cpu().numpy().mean(axis=1)
              #v_p = v_p.detach().cpu().numpy().mean(axis=1)
             # v_d_list.append(v_d)
             # v_p_list.append(v_p)
